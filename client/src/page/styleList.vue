        ...
        <div class="table_container">
            <el-table
                ...
                <el-table-column
                    ...
                    <template v-slot="slotProps">
                        <a :href="slotProps.row.url" onclick="return false;">{{ $route.meta }}}<i
...
export default {
    ...
    methods: {
        async initData() {
            try {
                console.log("初始化", this.kind)
                const countData = await getMusicCount({kind: this.kind});
        ...
        async getMusics() {
            const Musics = await getMusicList({kind: this.kind, startIndex: this.offset, pageNumber: this.limit});
            this.tableData = [];
